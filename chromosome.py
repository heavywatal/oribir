...
class Chromosome(object):
    ...
    _DIGITS = 2
    _NUCLEOTIDE = "ATGC"
    _CODEC = tuple(''.join(x) for x in itertools.product(_NUCLEOTIDE,
                                                         repeat=_DIGITS))
    ...
    def __init__(self, other=None):
        if not other:
            ...
            self.encode_([random.randint(0, 4 ** self.__class__._DIGITS)
    ...
    def __mul__(self, other):
        ...
        '''
        lambda_ = self.__class__.CROSSOVER_RATE * (len(self) - 1)
        n = numpy.random.poisson(lambda_)
    ...
    def _crossover(self, other, chiasma):
        (self._sequence, other._sequence) = (
            self[:chiasma] + other[chiasma:],
            other[:chiasma] + self[chiasma:])
        return

    def decode_(self):
        fun = self.__class__._CODEC.index
        return [fun(x) for x in split_(self._sequence, self.__class__._DIGITS)]
    ...
    def mutate(self):
        ...
        replace = self.__class__._NUCLEOTIDE.replace
        for i in numpy.random.randint(0, len(self), n):
            self[i] = random.choice(replace(self[i], ''))
        # # ALTERNATIVE ###
        # mu = self.__class__._MUTATION_RATE
        # trials = numpy.random.binomial(1, mu, len(self))
